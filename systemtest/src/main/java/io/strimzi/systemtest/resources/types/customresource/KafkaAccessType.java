...
public class KafkaAccessType implements ResourceType<KafkaAccess> {
    ...
    public KafkaAccessType() {
        client = KubeResourceManager.getKubeClient().getClient().resources(KafkaAccess.class);
    }
    ...
    public boolean isReady(KafkaAccess kafkaAccess) {
        KafkaAccessStatus kafkaAccessStatus = client.inNamespace(kafkaAccess.getMetadata().getNamespace()).resource(kafkaAccess).get().getStatus();
