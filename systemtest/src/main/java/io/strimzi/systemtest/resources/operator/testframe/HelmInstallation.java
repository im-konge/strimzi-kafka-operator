...
public class HelmInstallation implements InstallationMethod {
    ...
    public void install() {
        ...
        installCommand.set("watchAnyNamespace", clusterOperatorConfiguration.getNamespacesToWatch().equals(TestConstants.WATCH_ALL_NAMESPACES));
        installCommand.set("replicas", clusterOperatorConfiguration.getReplicas());

        if (!clusterOperatorConfiguration.getNamespacesToWatch().equals("*") && !clusterOperatorConfiguration.getNamespacesToWatch().equals(clusterOperatorConfiguration.getNamespaceName())) {
    ...
    public void delete() {
        ...
        KubeResourceManager.getKubeClient().getClient().namespaces().withName(clusterOperatorConfiguration.getNamespaceName()).delete();
    }
