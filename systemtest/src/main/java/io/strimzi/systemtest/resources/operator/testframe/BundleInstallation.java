...
public class BundleInstallation implements InstallationMethod {
    ...
    public void applyInstallationFiles() {
        List<File> operatorFiles = Arrays.stream(new File(CO_INSTALL_DIR).listFiles()).sorted()
            .filter(File::isFile)
            .filter(file ->
                !file.getName().matches(".*(Binding|Role)-.*"))
            .toList();
        ...
        for (File operatorFile : operatorFiles) {
            ...
            switch (resourceType) {
                ...
                case TestConstants.DEPLOYMENT:
                    deployClusterOperator(operatorFile);
                default:
    ...
    public void deployClusterOperator(File deploymentFile) {
        Deployment clusterOperator = TestFrameUtils.configFromYaml(deploymentFile, Deployment.class);
        ...
        for (EnvVar envVar : envVars) {
            switch (envVar.getName()) {
                case "STRIMZI_NAMESPACE":
                    envVar.setValue(clusterOperatorConfiguration.getNamespaceName());
        ...
        ResourceManager.getInstance().createResourceWithWait(new DeploymentBuilder(clusterOperator)
            .editMetadata()
                .withName(clusterOperatorConfiguration.getOperatorDeploymentName())
                .withNamespace(clusterOperatorConfiguration.getNamespaceName())
